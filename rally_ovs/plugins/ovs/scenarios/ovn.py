...
class OvnScenario(ovnclients.OvnClientMixin, scenario.OvsScenario):
    ...
    def _bind_ovs_internal_vm(self, lport, sandbox, flush, ovs_ssh):
        ...
        # TODO: containers don't have ethtool installed
    ...
    def _bind_ports(self, lports, sandboxes, port_bind_args):
        ...
        if (len(lports) < len(sandboxes)):
            for lport in lports:
                ...
                if internal:
                    ovs_ssh = self.farm_clients(farm, "ovs-ssh")
                    ovs_ssh.set_sandbox(sandbox, self.install_method, sandbox_data['host_container'])
                    ovs_ssh.enable_batch_mode()
            ...
            for i in range(0, len(lports), lport_per_sandbox):
                ...
                if internal:
                    ovs_ssh = self.farm_clients(farm, "ovs-ssh")
                    ovs_ssh.set_sandbox(sandbox, self.install_method, sandboxes[j]["host_container"])
                    ovs_ssh.enable_batch_mode()
    ...
    def _ping_port(self, lport, wait_timeout_s):
        ...
        ovs_ssh.set_sandbox(sandbox, self.install_method, sandbox['host_container'])
